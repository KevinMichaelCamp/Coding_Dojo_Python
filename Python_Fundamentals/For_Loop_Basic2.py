...
# 1. Biggie Size - Gievn an array, write a function that changes all positive numbers in the array to "big". Example: makeItBig([-1,3,5,-5]) returns that same array. changed to [-1,"big","big",-5].

# 2. Count Positives - Given an array of numbers, create a function to replace last value with number of positive valuse. Example, countPositives([-1,1,1,1]) changes the array to [-1,1,1,3] and returns it. (Note that zero is not considered to be a positive number).
